...
class NoCSRF{
    ...
    public static function check($key, $origin, $throwException = false, $timespan = null, $multiple = true){
        ...
        if(!isset($origin[$key]))
            if($throwException)
                throw new NoCSRFException('Missing CSRF form token.');
            else
                return false;
        ...
        if($timespan != null && is_int($timespan) && intval(substr(base64_decode($hash), 0, 10)) + $timespan < time())
            if($throwException)
                throw new NoCSRFException('CSRF token has expired.');
            else
                return false;
